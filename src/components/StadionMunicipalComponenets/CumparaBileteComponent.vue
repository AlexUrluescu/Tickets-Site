...
async function handlePickSeats(sector) {
  ...
  try {
    ...
    minRow.value = Math.min(...seats.value.map((s) => s.rand));
...
const rowRange = computed(() => {
  const range = [];
  for (let i = maxRow.value; i >= minRow.value; i--) {
    range.push(i);
  }
  return range;
});
...
<!-- DUPĂ -->
<div class="pb-2" style="max-height: 350px; overflow-x: auto; overflow-y: auto;">
  <div style="padding: 10px;" class="flex min-w-max">
                        <div
                        style="margin-right: 10px;"
                          class="grid gap-1 mr-2 text-xs font-bold text-gray-400 pt-1"
                          :style="{
                            gridTemplateRows: `repeat(${maxRow - minRow + 1}, 1fr)`,
                          }"
                        >
                          <div
                            v-for="r in rowRange"
