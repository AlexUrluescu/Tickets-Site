...
<template>
  ...
  >
    ...
    <div style="margin-top: 20px" class="w-full px-4 md:px-8 mt-6 mb-8">
      <div
        class="bg-white rounded-2xl shadow-md p-6 flex flex-col md:flex-row justify-between items-center gap-6 border-l-4"
      >
        <div class="flex items-center gap-3">
          <div
            style="color: #027be6; background-color: #96cbfa"
            class="p-3 rounded-full"
          >
            <img
              src="@/assets/images/Icons/clock_icon.png"
              alt="clock"
          ...
          <div>
            <p
              class="text-sm text-gray-500 font-medium uppercase tracking-wide"
            >
              Data & Ora
            </p>
        ...
        <div class="flex items-center gap-3">
          <div
            style="background-color: #96cbfa"
            class="p-3 rounded-full text-red-600"
          >
            <img
              src="@/assets/images/Icons/locatie_icon.png"
              alt="location"
              class="w-6 h-6 object-contain"
            />
          </div>
      ...
      <div class="flex flex-col">
        ...
        >
          <div class="absolute top-6 left-0 w-full text-center z-10">
            <h1
              class="text-2xl font-black text-gray-800 uppercase tracking-tight mb-1"
            >
              Harta Stadionului
            </h1>
            <p class="text-gray-500 text-sm">
              Alege sectorul dorit făcând click pe el
            </p>
          </div>

          <div
            class="w-full h-full flex items-center justify-center transform scale-100 md:scale-110 transition-transform duration-500 mt-10"
          >
            <svg
              width="401"
              height="302"
              ...
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              class="w-full h-auto max-w-lg drop-shadow-xl select-none font-sans"
            >
...
<template>
  <div
    style="width: 100%"
    class="min-h-screen bg-slate-50 flex flex-col font-sans pb-10"
  >
    <nav
      class="sticky top-0 z-30 w-full bg-white/90 backdrop-blur-md border-b border-gray-200 px-4 py-4 md:px-8 flex items-center justify-between"
    >
      <button
        @click="handleBack"
        class="group flex items-center gap-2 px-4 py-2 text-slate-600 hover:text-blue-600 font-bold transition-all"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
      ...
      <div class="hidden md:block">
        <h1 class="text-xl font-black text-slate-800 tracking-tight">
          MATCHDAY: <span class="text-blue-600">UNIV. CRAIOVA</span> VS
    ...
    <div
      ...
      class="w-full px-4 mt-8 grid grid-cols-1 lg:grid-cols-12 gap-8"
    >
      <div class="lg:col-span-7 flex flex-col">
        <div
          class="bg-white rounded-[2rem] shadow-xl border border-slate-100 p-8 flex flex-col items-center justify-center relative overflow-hidden min-h-[500px]"
        >
      ...
      <div class="lg:col-span-5 flex flex-col gap-4 relative">
        <h3
          class="text-xl font-bold text-slate-800 mb-2 pl-2 border-l-4 border-blue-600"
        >
          Alege Zona
        </h3>
                ...
                <Transition name="slide-fade">
                  ...
                  >
                    <div class="absolute top-2 right-2">
                      <button
                        @click="closeSeatPicker"
                        style="color: #027be6; cursor: pointer"
                        class="p-1 bg-gray-100 rounded-full text-gray-400 transition"
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          fill="none"
...
export const SeatPickerInternal = defineComponent({
  props: [
    "selectedSeats",
    "totalPrice",
    "maxRow",
  ...
  template: `
    <div class="bg-slate-50 rounded-xl p-4 border border-blue-100 relative">
      <div class="absolute top-2 right-2">
        <button style="color: #027be6" @click="closeSeatPicker" class="p-1 hover:bg-slate-200 rounded-full text-slate-400 transition">
      ...
      <div class="flex flex-wrap items-center justify-between gap-2 mb-4 pb-2 border-b border-slate-200">
        <div>
          <p class="text-xs text-slate-500 font-bold uppercase">Rezumat</p>
          <p class="text-sm font-bold text-slate-900">{{ selectedSeats }} Locuri • <span class="text-blue-600">{{ totalPrice }} RON</span></p>
        </div>
        <button class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-bold shadow-lg shadow-blue-200 transition transform active:scale-95" @click="addToCart">ADAUGĂ ÎN COȘ</button>
      </div>
      <div class="w-full overflow-x-auto pb-2 scrollbar-thin">
        <div class="flex min-w-max">
          <div class="grid gap-1 mr-2 text-[10px] font-bold text-slate-400 pt-1" :style="{ gridTemplateRows: \`repeat(\${maxRow}, 1fr)\` }">
            <div v-if="rowsArray" v-for="r in rowsArray" class="flex items-center justify-end h-6">R{{ r }}</div>
            <div v-else v-for="r in [...Array(maxRow).keys()].reverse()" class="flex items-center justify-end h-6">R{{ r + 1 }}</div>
          </div>
          <div class="grid gap-1" :style="{ gridTemplateRows: \`repeat(\${maxRow}, 1fr)\`, gridTemplateColumns: \`repeat(\${maxCol}, 1fr)\` }">
            <div v-for="seat in seats" :key="seat.rand + '-' + seat.loc" 
                 class="w-6 h-6 rounded-full flex items-center justify-center text-[9px] font-bold transition-all duration-150 border select-none shadow-sm"
                 :class="[
                   seat.status !== 'disponibil' ? 'bg-slate-200 text-slate-400 border-slate-200 cursor-not-allowed' : 
                   picked.has(\`\${seat.rand}-\${seat.loc}\`) ? 'bg-blue-600 text-white border-blue-700 scale-110 shadow-md ring-2 ring-blue-200' : 
                   'bg-white text-slate-700 border-slate-300 hover:border-blue-400 hover:bg-blue-50 cursor-pointer'
                 ]"
                 :style="{ gridRow: displayRow(seat.rand), gridColumn: seat.loc }"
                 @click="seat.status === 'disponibil' && toggleSeat(seat)">{{ seat.loc }}</div>
          </div>
        </div>
      </div>
      <div class="mt-4 text-center">
        <div class="h-8 bg-gradient-to-b from-green-500 to-green-700 rounded-md flex items-center justify-center text-white/90 font-black tracking-widest text-[10px] shadow-inner mb-3 w-3/4 mx-auto opacity-80">GAZON</div>
        <div class="flex justify-center gap-4 text-[10px] font-bold text-slate-500 uppercase tracking-wider">
          <div class="flex items-center gap-1"><div class="w-3 h-3 rounded-full bg-white border border-slate-300"></div> Liber</div>
          <div class="flex items-center gap-1"><div class="w-3 h-3 rounded-full bg-blue-600 border border-blue-700"></div> Selectat</div>
          <div class="flex items-center gap-1"><div class="w-3 h-3 rounded-full bg-slate-200 border border-slate-300"></div> Ocupat</div>
        </div>
      </div>
    </div>
  `,
...
.svg-selected {
  stroke: #1e40af !important; /* Blue 800 */
